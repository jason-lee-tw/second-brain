...
A 50-error, 12-file basedpyright remediation pass across the backend, fixed with three repeatable categories — targeted stub-gap ignores, per-node `TypedDict`s, and a shared content-narrowing helper — under a hard no-`Any`-in-owned-code rule.
