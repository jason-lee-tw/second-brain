...
- **Per-connection setup via pool `init=` hook, now consolidated in `db/pool.py`**: the asyncpg pool's `init` callback (`_setup_conn`) runs once per new pooled connection and performs two registrations: `await register_vector(conn)` (pgvector type codec, from `pgvector.asyncpg`) and `await conn.set_type_codec("jsonb", encoder=json.dumps, decoder=json.loads, schema="pg_catalog")` (JSONB codec). Codec registration is a pool-initialization concern, distinct from query-time logic — every connection the pool ever creates goes through the same `init` coroutine. Chronologically, the JSONB codec was first added directly inside `rag_retrieval.py`'s then node-local pool (the 2026-06-25 fix); the ticket-5 grilling session on 2026-06-26 (decision D1) then consolidated the pool itself — `_setup_conn`, `register_vector`, and the jsonb codec together — into the shared `db/pool.py` singleton, superseding the rag_retrieval.py-local location.
