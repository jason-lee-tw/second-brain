...
async def test_ac5_pii_redacted_before_llm_sees_message():
  ...
  with (
    ...
    mock_memory_agent_llm.ainvoke = AsyncMock(return_value=_mock_memory_agent_output())

    graph, _pool = await build_query_graph("postgresql://fake:test@localhost:5432/test")
    ...
    with (
      ...
      async with AsyncClient(
        ...
        assert data2["sessionId"] == session_id, (
          f"Second call must echo back the same sessionId; got {data2['sessionId']!r}"
        )
